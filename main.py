...
def parse_arguments():
    ...
    parser.add_argument("--lr", default=0.0001, type=float,
                        help="Learning rate for TSignal (not ProtBERT).")
    parser.add_argument("--anneal_start", default=-1, type=int,
                        help="Epoch where learning rate starts to be lowered.")
    parser.add_argument("--anneal_epochs", default=-1, type=int,
                        help="Number of epochs in which to reach the desired annealed_lr")
    parser.add_argument("--annealed_lr", default=0.00002,
                        type=float, help="Desired annealed_lr")
    ...
    parser.add_argument("--run_name", default="some_run", type=str,
                        help="Name your run. This will be used as a prefix for all saved files: logs, models, etc.")
    ...
    parser.add_argument("--dropout", default=0, type=float,
                        help="Dropout on all TSignal layers (not ProtBERT).")
    ...
    parser.add_argument("--nheads", default=8, type=int,
                        help="Number of heads to be used (same as paranthesis for nlayers)")
    ...
    parser.add_argument("--train_oh", default=False, action="store_true",
                        help="Use one-hot encoding instead of ProtBERT")
    ...
    parser.add_argument("--test_seqs", default="", type=str,
                        help="filename of the binary containing sequences that will be tested. File needs to be in sp_data/ folder")
    parser.add_argument("--test_mdl", default="", type=str,
                        help="filename of model that will be tested on test_seqs file")
    ...
    parser.add_argument("--ff_d", default=4096, type=int,
                        help='Expanding dimension')
    ...
    parser.add_argument("--validate_on_test",
                        default=False, action="store_true")
    parser.add_argument("--form_sp_reg_data",
                        default=False, action="store_true")
    ...
    parser.add_argument("--separate_save_sptype_preds",
                        default=False, action="store_true")
    ...
    parser.add_argument("--weight_class_loss",
                        default=False, action="store_true")
    parser.add_argument("--weight_lbl_loss",
                        default=False, action="store_true")
    parser.add_argument("--account_lipos", default=False, action="store_true")
    parser.add_argument("--tuned_bert_embs",
                        default=False, action="store_true")
    parser.add_argument("--tune_bert", default=False,
                        action="store_true", help="Tune BERT and TSignal together")
    parser.add_argument("--frozen_epochs", default=3, type=int)
    parser.add_argument("--frozen_pe_epochs", default=-1, type=int)
    parser.add_argument("--no_bert_pe_training",
                        default=False, action="store_true")
    parser.add_argument("--extended_sublbls",
                        default=False, action="store_true")
    parser.add_argument("--random_folds", default=False, action="store_true")
    parser.add_argument("--train_on_subset", default=1., type=float)
    parser.add_argument("--train_only_decoder",
                        default=False, action="store_true")
    parser.add_argument("--remove_bert_layers", default=0, type=int)
    parser.add_argument("--augment_trimmed_seqs",
                        default=False, action="store_true")
    parser.add_argument("--saliency_map_save_fn", default="save.bin", type=str)
    parser.add_argument("--compute_saliency",
                        default=False, action="store_true")
    parser.add_argument("--hook_layer", default="bert", type=str)
    parser.add_argument("--cycle_length", default=5, type=int)
    parser.add_argument("--bert_pe_for_decoder",
                        default=False, action="store_true")
    parser.add_argument("--concat_pos_enc", default=False, action="store_true")
    parser.add_argument("--pe_extra_dims", default=64, type=int)
    parser.add_argument("--add_bert_pe_from_dec_to_bert_out",
                        default=False, action="store_true")
    parser.add_argument("--lr_multiplier_swa", default=1,
                        type=int, help="number to multiply the maximum swa lr")
    parser.add_argument("--change_swa_decoder_optimizer", default=False, action="store_true",
                        help="change decoder optimizer when starting swa from Adam to SGD")
    parser.add_argument("--reinint_swa_decoder", default=False, action="store_true",
                        help="Use the same (Adam) optimizer when SWA starts, but reinitialize it")
    parser.add_argument("--add_val_data_on_swa", default=False, action="store_true",
                        help="SWA isnt based on early stopping, so the validation data can also be used for training")
    parser.add_argument("--lipbobox_predictions", default=False, action="store_true",
                        help="Modify predictions to have SSS...(SSS)LLL for Sec/SPase (I)II and TTT...(TTT)LLL for Tat/SPase (I)II")
    parser.add_argument("--train_sp_type_predictor", default=False, action="store_true",
                        help="train a TCR conv architecture to predict the sp type")
    parser.add_argument("--load_model", default="none", type=str,
                        help="Option to load any bert model (e.g. trained on SP-CS prediction task)")
    parser.add_argument("--deep_mdl", default=False, action="store_true",
                        help="Add another layer before the class head of the sp type model")
    parser.add_argument("--is_cnn2", default=False, action="store_true",
                        help="Remove the additional linear layer.")
    parser.add_argument("--no_of_layers_onlysp", default=8,
                        type=int, help="No. of additional layers sp classifier.")
    ...
    parser.add_argument("--no_of_layers_conv_resnets", default=4,
                        type=int, help="No. of additional (conv) layers sp classifier.")
    parser.add_argument("--is_cnn4", default=False,
                        action="store_true", help="Use new cnn4 architecture")
    parser.add_argument("--use_sgd_on_swa", default=False,
                        action="store_true", help="Use sgd on swa for sp type classifier")
    parser.add_argument("--swa_start", default=60, type=int,
                        help="Epoch at wich you load best mdl and start swa")
    parser.add_argument("--og_emb_dim", default=32, type=int,
                        help="Epoch at wich you load best mdl and start swa")
    parser.add_argument("--residue_emb_extra_dims", default=0,
                        type=int, help="Epoch at wich you load best mdl and start swa")
    parser.add_argument("--add_extra_embs2_decoder", default=False, action="store_true",
                        help="Add the extra decoder inputs to the Transformer Decoder (this way, the final layer W_O still won't have access to non-contextualized residue representations")
    ...
    parser.add_argument("--use_aa_len", default=200, type=int,
                        help="When training binary classifiers (not for TSignal).")
...
def modify_param_search_args(args):
    ...
    if args.validate_partition is not None:
        run_name += "_t_{}_v_{}".format(
            args.train_folds[0], args.validate_partition)
    else:
        run_name += "trFlds_{}_{}".format(
            args.train_folds[0], args.train_folds[1])
...
if __name__ == "__main__":
    ...
    if args.test_seqs:
        test_seqs_w_pretrained_mdl(args.test_mdl, args.test_seqs, tune_bert=args.tune_bert,
                                   saliency_map_save_fn=args.saliency_map_save_fn, hook_layer=args.hook_layer, compute_saliency=args.compute_saliency)
    elif args.train_sp_type_predictor:
        logging.basicConfig(filename=args.run_name +
