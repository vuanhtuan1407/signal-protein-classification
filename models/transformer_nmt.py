    ...
    def forward(self, tokens: Tensor):
        ...
        if self.lbl2ind is not None:
            ...
            for tk_seq in tokens:
                ...
                tk_tensor.extend([self.lbl2ind["PD"]] *
                                 (max_len - len(tk_seq)))
...
class InputEmbeddingEncoder(nn.Module):
    def __init__(self, partitions=[0, 1, 2], og2ind=None, use_glbl_lbls=False, aa2ind=None,
        ...
        if residue_emb_extra_dims != 0:
            if use_blosum:
                self.extra_embs_dec_input = pickle.load(
                    open("blusum_m.bin", "rb"))
            else:
                self.extra_embs_dec_input = nn.Embedding(
                    20, residue_emb_extra_dims)
                aa_dict = pickle.load(open("sp6_dicts.bin", "rb"))
                aa_dict = {
                    k: v for k, v in aa_dict[-1].items() if v not in ['ES', 'PD', 'BS']}
                aa_dict['X'] = 20
                self.aa2ind_extra = aa_dict
                self.extra_emb_layer_norm = nn.LayerNorm(
                    residue_emb_extra_dims)
        else:
        ...
        if aa2ind is None:
            ...
            for p in partitions:
                for t in ["test", "train"]:
                    ...
                    if not tuning_bert:
                        seq2emb.update(
                            {seq: emb for seq, (emb, _, _, _) in part_dict.items()})
                    if og2ind is not None:
                        seq2lg.update(
                            {seq: og2ind[lg] for seq, (_, _, lg, _) in part_dict.items()})
            self.seq2lg = seq2lg
            ...
            self.eos_bos_cls_embs = TokenEmbedding(
                3, 1024) if use_glbl_lbls else TokenEmbedding(2, 1024)
            self.lg_embs = TokenEmbedding(
                len(og2ind.keys()), 1024) if og2ind is not None else None
    ...
    def create_empty_mask(self, shape):
        ...
        src_mask = torch.zeros(
            (shape, shape), device=self.device).type(torch.bool)
        return src_mask
    ...
    def add_bos_eos_lg_glb_cls_tkns(self, strseq_or_inpemb, inp_seq=None):
        ...
        aa_embedding = torch.tensor(
            self.seq2emb[strseq_or_inpemb], device=self.device) if inp_seq is None else strseq_or_inpemb
        ...
        if self.use_glbl_lbls and self.glbl_lbl_version == 1:
            cls_emb = self.eos_bos_cls_embs(
                torch.tensor(2, device=self.device)).reshape(1, -1)
        ...
        if self.extra_embs_dec_input:
            if self.use_blosum:
                ...
                extra_emb_tensor = torch.cat([torch.tensor(
                    [self.extra_embs_dec_input[r] for r in inp_extra_emb], device=self.device, dtype=torch.float32)])
                ...
                inp_extra_emb = inp_seq if not self.use_lg else inp_seq + "X"
                extra_emb_tensor = self.extra_emb_layer_norm(self.extra_embs_dec_input(
                    torch.tensor([self.aa2ind_extra[r] for r in inp_extra_emb], device=self.device)))
                return torch.cat([torch.cat(input_tensor, dim=0), extra_emb_tensor], dim=1)
        return torch.cat(input_tensor, dim=0)

    def make_oh(self, seq):
        inds = torch.tensor([self.aa2ind_extra[r_]
                            for r_ in seq], device=self.device)
    ...
    def oh_forward(self, seqs):
        ...
        for s in seqs:
            ...
            tensor_inputs.append(self.emb_layer(
                torch.tensor(current_s, device=self.device)))
        ...
        if self.use_glbl_lbls:
            additional_inp_tkns += 1
        output_lens = [ti.shape[0] -
                       additional_inp_tkns for ti in tensor_inputs]
        max_len = max(input_lens)
        max_len_out = max(output_lens)
        padding_mask_src = torch.arange(
            max_len)[None, :] < torch.tensor(input_lens)[:, None]
        # src and padding are always of the same size
        padding_mask_tgt = torch.arange(
            max_len_out)[None, :] < torch.tensor(output_lens)[:, None]
    ...
    def forward(self, seqs, inp_seqs=None):
        ...
        if inp_seqs is not None:
            tensor_inputs = [self.add_bos_eos_lg_glb_cls_tkns(
                s, inp_seq=is_) for (s, is_) in zip(seqs, inp_seqs)]
        else:
            tensor_inputs = [self.add_bos_eos_lg_glb_cls_tkns(s) for s in seqs]
        if inp_seqs is None:
            input_lens = [ti.shape[0] for ti in tensor_inputs]
        else:
            input_lens = [len(i) if not self.use_lg else len(
                i) + 1 for i in inp_seqs]
        # additional inputs BOS tokens, glbl_lbl (<cls> token), and lg
        additional_inp_tkns = 1
        if self.use_lg:
            additional_inp_tkns += 1
        if self.use_glbl_lbls and self.glbl_lbl_version == 1:
            additional_inp_tkns += 1
        output_lens = [ti.shape[0] -
                       additional_inp_tkns + 2 for ti in tensor_inputs]
        max_len = max(input_lens)
        max_len_out = max(output_lens)
        padding_mask_src = torch.arange(
            max_len)[None, :] < torch.tensor(input_lens)[:, None]
    ...
    def update(self, partitions=[0, 1, 2], emb_f_name=None, tuned_bert_embs_prefix=""):
        ...
        if emb_f_name is not None:
            ...
            self.seq2lg = {seq: self.og2ind[lg]
                           for seq, (_, _, lg, _) in dict_.items()}
        else:
            for p in partitions:
                for t in ["test", "train"]:
                    part_dict = pickle.load(open(self.data_folder + tuned_bert_embs_prefix +
...
class TransformerModel(nn.Module):
    ...
    def __init__(self, ntoken: int, d_model: int, nhead: int, d_hid: int, nlayers: int, dropout: float = 0.5,
        ...
        aa_dict = pickle.load(open("sp6_dicts.bin", "rb"))
        aa_dict = {k: v for k,
                   v in aa_dict[-1].items() if v not in ['ES', 'PD', 'BS']}
        aa_dict['X'] = 20
        self.aa2ind_extra = aa_dict
        self.use_blosum = use_blosum
        self.use_extra_oh = use_extra_oh
        ...
        if use_extra_oh:
            ...
            self.extra_embs_gen_input = pickle.load(
                open(get_data_folder()+"blosum.bin", "rb"))
        else:
        ...
        self.device = torch.device(
            "cuda:0" if torch.cuda.is_available() else "cpu")
        self.pos_encoder = PositionalEncoding(d_model, dropout=dropout if input_drop else 0, linear_pos_enc=linear_pos_enc, concat_pos_enc=concat_pos_enc, pe_extra_dims=pe_extra_dims,
        ...
        if not train_only_decoder:
            ...
            decoder_norm = LayerNorm(
                d_hid + self.pe_extra_dims + extra_dims_decoder)

            self.transformer = TransformerDecoder(
                decoder_layer, num_layers=nlayers, norm=decoder_norm).to(self.device)
        ...
        self.label_encoder = TokenEmbedding(
            ntoken, d_hid + extra_dims_decoder, lbl2ind=lbl2ind)
        ...
        self.generator = nn.Linear(
            d_model + self.pe_extra_dims + residue_emb_extra_dims, ntoken).to(self.device)
        self.use_glbl_lbls = use_glbl_lbls
        self.glbl_lbl_version = glbl_lbl_version
        if self.form_sp_reg_data and not use_glbl_lbls:
            self.glbl_generator = nn.Linear(
                ntoken, no_glbl_lbls).to(self.device)
        elif use_glbl_lbls and self.glbl_lbl_version != 3 and self.glbl_lbl_version != 4:
            self.glbl_generator = nn.Linear(
                d_model, no_glbl_lbls).to(self.device)
        elif self.use_glbl_lbls and self.glbl_lbl_version == 3 and self.glbl_lbl_version != 4:
            self.glbl_generator = CNN3(
                input_size=1024, output_size=no_glbl_lbls).to(self.device)
    ...
    def encode(self, src, inp_seqs=None):
        ...
        src_mask, tgt_mask, padding_mask_src, padding_mask_tgt, src = self.input_encoder(
            src, inp_seqs=inp_seqs)
        if len(src) == 1:
            src = src[0].reshape(1, *src[0].shape).transpose(0, 1)
        else:
            src = torch.nn.utils.rnn.pad_sequence(
                src, batch_first=True).transpose(0, 1)
    ...
    def forward_glb_lbls(self, src, tgt):
        ...
        padded_src = self.pos_encoder(
            padded_src.transpose(0, 1), scale=self.scale_input)
        padded_tgt = torch.nn.utils.rnn.pad_sequence(
            self.label_encoder(tgt), batch_first=True).to(self.device)
        padded_tgt = self.pos_encoder(padded_tgt.transpose(0, 1))
        # [ FALSE FALSE ... TRUE TRUE FALSE FALSE FALSE ... TRUE TRUE ...]
        memory = self.transformer.encoder(
            padded_src, src_mask, padding_mask_src)
        outs = self.transformer.decoder(
            padded_tgt, memory, tgt_mask, tgt_key_padding_mask=padding_mask_tgt)
    ...
    def get_v3_glbl_lbls(self, src, inp_seqs=None):
        ...
        if self.add_lg_info:
            ...
            src_for_glbl_l = [src[i][trim_ind_l:-trim_ind_r, :]
                              for i in range(len(src))]
        else:
            trim_ind_l, trim_ind_r = 0, 0
            src_for_glbl_l = [src[i] for i in range(len(src))]
        padded_src_glbl = torch.nn.utils.rnn.pad_sequence(
            src_for_glbl_l, batch_first=True)
    ...
    def forward_only_decoder(self, src, tgt, inp_seqs, tgt_mask=None, padding_mask_src=None):
        if tgt_mask is None:
            ...
            if padding_mask_src is not None:
                src_mask, _, _, padding_mask_tgt, _ = self.input_encoder(
                    src, inp_seqs=inp_seqs)
            else:
                src_mask, _, padding_mask_src, padding_mask_tgt, _ = self.input_encoder(
                    src, inp_seqs=inp_seqs)
        ...
        padded_src = self.pos_encoder(padded_src.transpose(
            0, 1), scale=self.scale_input, no_pos_enc=no_pos_enc, add_lg_info=self.add_lg_info)
        # [ FALSE FALSE ... TRUE TRUE FALSE FALSE FALSE ... TRUE TRUE ...]
    ...
    def forward(self, src: Tensor, tgt: list, inp_seqs=None) -> Tensor:
        ...
        padded_src = torch.nn.utils.rnn.pad_sequence(src, batch_first=True)
        padded_src = self.pos_encoder(padded_src.transpose(
            0, 1), scale=self.scale_input, no_pos_enc=self.no_pos_enc)
        padded_tgt = torch.nn.utils.rnn.pad_sequence(
            self.label_encoder(tgt), batch_first=True).to(self.device)
        padded_tgt = self.pos_encoder(padded_tgt.transpose(0, 1))
        # [ FALSE FALSE ... TRUE TRUE FALSE FALSE FALSE ... TRUE TRUE ...]
...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model: int, dropout: float = 0.1, max_len: int = 5000, linear_pos_enc=False, concat_pos_enc=False, pe_extra_dims=64,
        ...
        pe = torch.zeros(max_len, 1, d_model).to(self.device) if not concat_pos_enc else torch.zeros(
            max_len, 1, pe_extra_dims).to(self.device)
        ...
        if self.linear_pos_enc and concat_pos_enc:
            ...
            self.pos_enc = torch.nn.functional.one_hot(a, num_classes=pe_extra_dims).reshape(
                pe_extra_dims, 1, pe_extra_dims).to(self.device)
    ...
    def forward(self, x: Tensor, scale=False, no_pos_enc=False, add_lg_info=False) -> Tensor:
        ...
        if self.linear_pos_enc and not no_pos_enc:
            pos_enc = self.pos_enc(torch.tensor(
                list(range(x.shape[0]))).to(self.device))
            pos_enc = pos_enc.repeat(1, x.shape[1]).reshape(
                x.shape[0], x.shape[1], x.shape[2])
            if scale:
                x = self.dropout(x * np.sqrt(1024) +
                                 self.pe[:x.size(0)] + pos_enc)
                return x
            else:
        ...
        if scale:
            ...
            if add_lg_info:
                ...
                pe_ = torch.cat(
                    [pe_, torch.zeros(1, 1, pe_.shape[-1]).to(self.device)], dim=0)
